...
async function connectMongo() {
  try {
    ...
    console.warn("   Make sure MongoDB / Compass is running on localhost:27017");
...
async function seedAdminAccount() {
  ...
  console.log("   🛡️  Admin account seeded:", adminEmail, "/ password:", adminPassword);
  console.log("   ⚠️  Set ADMIN_PASSWORD in .env before going live!");
}
...
app.post("/api/contact", async (req, res) => {
  ...
  if (contactCollection) {
    try {
      await contactCollection.insertOne(payload);
    } catch (e) {
      return res.status(500).json({ success: false, message: "Server error." });
    }
  } else {
    memContact.push({ ...payload, id: "msg_" + Date.now() });
  }
  res.json({ success: true });
});

// ── ADMIN ROUTES ─────────────────────────────────────────────────────────────
// The ADMIN_EMAIL in .env (or fallback) is the ONLY email that can access these.
...
// POST /api/admin/check — verify if a user is admin
app.post("/api/admin/check", (req, res) => {
  const { email } = req.body;
  res.json({ isAdmin: isAdmin(email) });
});

// POST /api/admin/users — get all users (admin only)
...
// POST /api/admin/messages — get all contact messages (admin only)
...
// POST /api/admin/grant-yearly — grant Plus badge to a user
...
// POST /api/admin/revoke-yearly — revoke Plus badge
...
app.post("/api/admin/messages", async (req, res) => {
  ...
  if (usersCollection) {
    try {
      ...
      const messages = await contactsCol.find({}).sort({ createdAt: -1 }).toArray();
      return res.json({ messages });
...
app.post("/api/contact", async (req, res) => {
  ...
  const entry = { name, email, subject: subject || "general", message, createdAt: new Date(), read: false };
  if (usersCollection) {
    try {
      const contactsCol = db.collection("contacts");
      await contactsCol.insertOne(entry);
      console.log(`📬 Contact message from ${email}: [${subject}]`);
    } catch (err) {
    ...
    // In-memory fallback
    console.log("📬 Contact (in-memory):", { name, email, subject, message: message.slice(0, 60) });
  }
  res.json({ success: true });
});

// NOTE: This key is kept server-side (not exposed to the browser).
// For best practice, set SPOONACULAR_API_KEY in your environment.
const SPOONACULAR_API_KEY =
  process.env.SPOONACULAR_API_KEY || "c29c92dbbbc443abb04b87eead98074b";
...
app.get("/api/meals", async (req, res) => {
  try {
    ...
    // Convert Spoonacular recipes into “deliverable meal options”
...
app.post("/api/generate-meal-plan", async (req, res) => {
  ...
  try {
    ...
    // Removed strict API key check to allow fallback for users without keys
...
JSON format:
{
  "meals": [
    ...
    try {
      ...
      console.log(`✅ Gemini 3.1 Flash Lite generated ${mealPlan?.meals?.length} meals.`);
    } catch (parseError) {
      console.warn("⚠️ Failed to parse Gemini JSON:", parseError.message);
      console.warn("Raw Gemini response:", response.text?.slice(0, 500));
      mealPlan = {};
    }
    ...
    if (!mealPlan.meals || !Array.isArray(mealPlan.meals)) {
      ...
      console.warn(`Gemini returned ${mealPlan.meals.length} meals instead of ${mealCount}. Truncating or padding.`);
    ...
    // Algorithmic fallback when AI completely fails
    res.json({ meals: generateAlgorithmicMeals(profile, health, mealCount) });
  }
});

// Shared algorithmic meal generator — used as primary fallback
function generateAlgorithmicMeals(profile, health, count = 3) {
  ...
  // Extreme fallback if user is allergic to EVERYTHING in a list
  ...
  // Base price heuristic: higher protein meals cost a bit more
  const basePrices = {
  ...
  for (let i = 0; i < count; i++) {
    const pName = proteins[i % proteins.length];
    const bName = bases[i % bases.length];
    ...
    if (health?.allergies && health.allergies.toLowerCase() !== 'none') {
      instruction += ` Free of ${health.allergies}.`;
    }
    if (health?.issues && health.issues.length > 0) {
      instruction += ` Prepared for ${health.issues.join(', ')}.`;
    }
...
{"name":"Recipe Name","image":"https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=600","instructions":"Brief 1-2 sentence recipe overview.","prepTime":20,"calories":450}`;
    ...
    console.error("Gemini Idea API Error details:", {
      message: error.message,
      status: error.status,
      response: error.response?.data
    });

    // Fallback: Use generateAlgorithmicMeals to provide a consistent response if Gemini is unavailable
